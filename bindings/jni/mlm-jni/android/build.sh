...
echo "********  Building Malamute JNI interface & classes"
( cd ../.. && TERM=dumb ./gradlew build jar -PbuildPrefix=$BUILD_PREFIX --info )

echo "********  Building Malamute JNI for Android"
...
find ../../build/libs/ -type f -name 'mlm-jni-*.jar' ! -name '*javadoc.jar' ! -name '*sources.jar' -exec unzip -q {} +
...
cp libmlmjni.so lib/$TOOLCHAIN_ABI
